...
{
	const UEnum* PoseTypeEnum = FindObject<UEnum>(ANY_PACKAGE, TEXT("EIdentityPoseType"));
	// TODO: Replace this with GetDisplayNameTextByValue so it can be localized
	return PoseTypeEnum->GetNameStringByValue(static_cast<int64>(InPoseType));
}
...
FSlateIcon UMetaHumanIdentityPose::GetPoseIcon() const
{
	...
	const FName PoseIconName = FEditorStyle::Join("Identity.Pose.", PoseTypeNamePtr);
...
void UMetaHumanIdentityFace::ApplyDNAToRig(TArray<uint8>& InDNABuffer, bool bInUpdateBlendShapes, bool bInUpdateSkinWeights)
{
	...
	if (RigComponent != nullptr && RigComponent->SkeletalMesh->GetSkeleton() != nullptr)
	{
		...
		FDNAToSkelMeshMap* DNAToSkelMeshMap = USkelMeshDNAUtils2::CreateMapForUpdatingNeutralMesh(DNAReader.Get(), RigComponent->SkeletalMesh);
		...
		RigComponent->SkeletalMesh->AddAssetUserData(DNAAsset);

		USkelMeshDNAUtils2::UpdateJoints(RigComponent->SkeletalMesh, DNAReader.Get(), DNAToSkelMeshMap);
		USkelMeshDNAUtils2::UpdateBaseMesh(RigComponent->SkeletalMesh, DNAReader.Get(), DNAToSkelMeshMap, ELodUpdateOption2::LOD0Only);

		if (!bInUpdateBlendShapes)
		{
			USkelMeshDNAUtils2::RebuildRenderData_VertexPosition(RigComponent->SkeletalMesh);
		}

		if (bInUpdateSkinWeights)
		{
			USkelMeshDNAUtils2::UpdateSkinWeights(RigComponent->SkeletalMesh, DNAReader.Get(), DNAToSkelMeshMap, ELodUpdateOption2::LOD0Only);
		}

		if (bInUpdateBlendShapes)
		{
			// we know that blend shapes exist only for LOD 0, so here we ignore the Options.LODsToInclude
			USkelMeshDNAUtils2::UpdateMorphTargets(RigComponent->SkeletalMesh, DNAReader.Get(), DNAToSkelMeshMap, ELodUpdateOption2::LOD0Only);
			USkelMeshDNAUtils2::RebuildRenderData(RigComponent->SkeletalMesh);
		}

		// Source data must be updated if cooking
		USkelMeshDNAUtils2::UpdateSourceData(RigComponent->SkeletalMesh, DNAReader.Get(), DNAToSkelMeshMap);
...
{
	if (RigComponent != nullptr && RigComponent->SkeletalMesh != nullptr && RigComponent->SkeletalMesh->GetSkeleton() == nullptr)
	{
		...
		if (USkeletalMesh* TemplateRig = Cast<USkeletalMesh>(StaticLoadObject(USkeletalMesh::StaticClass(), nullptr, TemplateRigPath)))
		{
			...
			if (RigComponent->SkeletalMesh == nullptr || RigComponent->SkeletalMesh->GetSkeleton() == nullptr)
			{
